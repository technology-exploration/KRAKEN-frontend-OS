...
export function metric1_calculator(num_files, adv_strength_min, adv_strenght_max) {
    ...
    const val1 = num_files - (num_files * adv_strength_min)
    const val2 = num_files - (num_files * adv_strenght_max)
    const file_access = num_files - ((val1 + val2) / 2.0)
...
export function metric2_calculator(num_files, sim_files, privacy_val) {
    ...
    const file_access = Math.round(num_files * (privacy_val / 100.0))
    ...
    const temp_files = num_files - sim_files
    let total = 0

    for (let i = 2; i < sim_files + 1; i++) {
        const temp_access = file_access - i
    ...
    const dividend = binomial.get(num_files, file_access)
...
function adversary(input) {
    ...
    const strength_min = adv_strength_min(strength)
    const strength_max = adv_strength_max(strength)
...
export default function computePrivacyMetric(input) {
    ...
    const metric1_res = adversary(input)
    let health = 6,
        location = 6,
        job = 6,
        other = 6
    ...
    if (input.checkAge || input.checkBody || input.checkGender || input.checkHealth || input.checkSports) {
        ...
        const scale = get_scale_desc(input.q2)
    ...
    if (input.checkCity || input.checkCountry || input.checkGPS) {
        ...
        // scale the privacy value individual
        const scale = get_scale_desc(input.q3)
    ...
    if (input.checkDegree || input.checkGrades || input.checkJob) {
        ...
        // scale the privacy value individual
        const scale = get_scale_desc(input.q4)
    ...
    if (input.checkConnection || input.checkOther) {
        const other_res = Math.min(100, metric2_calculator(input.numFiles, counter[3], metric1_res)) // range [0,100]
        other = scale_grade(other_res) // range [1,6]
    }

    // calculate the overall privacy value based on these 4 categories
    const privacy = Math.min(6, (job + health + location + other) / 4.0) // range [1,6]
