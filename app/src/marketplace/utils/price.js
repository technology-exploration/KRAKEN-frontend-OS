 ...
 * @param dataPerUsd Number of DATA units per 1 EUR.
 */
export const dataToEur = (data: BN, dataPerUsd: BN): BN => (BN(dataPerUsd).isZero() ? BN(0) : BN(data).dividedBy(dataPerUsd))

/**
 * Convert EUR to DATA.
 * @param eur Number of EUR to convert.
 * @param dataPerUsd Number of DATA units per 1 EUR.
 */
export const eurToData = (eur: BN, dataPerUsd: BN): BN => BN(eur).multipliedBy(dataPerUsd)
...
export const convert = (amount: BN, dataPerUsd: BN, fromCurrency: ContractCurrency, toCurrency: ContractCurrency): BN => {
    ...
    const calc = fromCurrency === contractCurrencies.DATA ? dataToEur : eurToData
 ...
 */
export const formatPrice = (pricePerSecond: BN, currency: PaymentCurrency | ContractCurrency, timeUnit?: TimeUnit, amountTime?: number): string => {
    ...
    const roundedPriceTotal = formatDecimals(price, currency) * amountTime
    return amountTime ? `${roundedPriceTotal} ${currency}` : `${roundedPrice} ${currency} / ${timeUnitAbbreviation}`
}

export const formatPriceNoTimeUnit = (pricePerSecond: BN, currency: PaymentCurrency | ContractCurrency, timeUnit?: TimeUnit, amountTime?: number): string => {
    const actualTimeUnit = timeUnit || getMostRelevantTimeUnit(pricePerSecond)
    const price = priceForTimeUnits(pricePerSecond, 1, actualTimeUnit)
    const roundedPrice = formatDecimals(price, currency)
    const roundedPriceTotal = formatDecimals(price, currency) * amountTime
    return amountTime ? `${roundedPriceTotal} ${currency}` : `${roundedPrice} ${currency}`
}
