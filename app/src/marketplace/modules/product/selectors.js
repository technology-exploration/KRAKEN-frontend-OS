...
export const selectProductIsPending: (state: StoreState) => boolean = createSelector(
    selectProductId,
    selectSubscriptions,
    (productId, subscriptions) => {
        if (!productId) {
        ...
        const subscription = subscriptions && subscriptions.find((s) => s.id === productId)
        return !!subscription && (subscription.pending || '')
    },
)

// export const selectProductPending: (state: StoreState) => boolean = createSelector(
//     selectProductId,
//     selectSubscriptions,
//     (productId, subscriptions) => {
//         if (!productId) {
...
//         }
//         const subscription = subscriptions && subscriptions.find((s) => s.id === productId)
//         return !!subscription && isActive(subscription.pending || '')
//     },
// )
