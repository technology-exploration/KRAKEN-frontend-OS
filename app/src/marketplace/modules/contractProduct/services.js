...
const createContractProductWithoutWhitelist = (product: SmartContractProduct): SmartContractTransaction => {
    ...
    let transformedPricePerSecond
    if (product.type === '1') {
        transformedPricePerSecond = 1
    } else {
        transformedPricePerSecond = mapPriceToContract(pricePerSecond)
    }
...
export const buyAnalyticsPackage = (ids: Array<ProductId>): SmartContractTransaction => (
    send(contractMethods().buyAnalyticsPackage(ids.map((id) => getValidId(id))))
)
