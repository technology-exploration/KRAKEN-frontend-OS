...
const UnstyledNew = ({ currentUser, ...props }) => {
    ...
    const loadDomains = useCallback(async () => {
        ...
        const addresses = (!!username && isEthereumAddress(address)) ? [address] : []
    ...
    }, [address, domains, isMounted, loadingDomains])
    ...
    useEffect(() => {
        ...
        // const { username } = currentUserRef.current || {}
        // const addresses = (!!currentUser && isEthereumAddress(address)) ? [address] : []
        // const addresses = ethereumIdentitiesRef.current.map(({ json }) => json.address)
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [domain, currentUser, domainOptions, updateStream, address])
                                    ...
                                    >
                                        {!!isDomainDisabled && (
                                            <DisabledDomain>
                                                {address
                                                 && (
                                                     <span className="text-center">{truncate(domain, 16)}</span>
                                                 )}
                                                {!!loadingDomains && !address && (
                                                    <React.Fragment>
