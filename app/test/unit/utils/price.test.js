...
describe('price utils', () => {
    ...
    describe('dataToEur', () => {
        it('converts currency with given rate', () => {
            assert.equal(all.dataToEur(10, 0), 0)
            assert.equal(all.dataToEur(0, 10), 0)
            assert.equal(all.dataToEur(1, 0.5), 2)
            assert.equal(all.dataToEur(1, 4), 0.25)
        })
    })

    describe('eurToData', () => {
        it('converts currency with given rate', () => {
            assert.equal(all.eurToData(10, 0), 0)
            assert.equal(all.eurToData(0, 10), 0)
            assert.equal(all.eurToData(1, 0.5), 0.5)
            assert.equal(all.eurToData(10, 80), 800)
        })
    })
